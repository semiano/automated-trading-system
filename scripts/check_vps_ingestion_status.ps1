...
docker compose --env-file .env.docker logs --since 5m ingestion | grep -E "Heartbeat synthesized|Synthesized [0-9]+ missing 1m candles|Coinbase WS connected|queue full|WS error|WS closed" | tail -n 80 || true
