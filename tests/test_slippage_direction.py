...
def test_fees_reduce_round_trip_pnl():
    ...
    entry_no_fee = adapter.submit_entry(symbol="XRP/USDT", raw_price=100.0, qty=1.0, trade_side="long", constraints=no_fee)
    exit_no_fee = adapter.submit_exit(symbol="XRP/USDT", raw_price=101.0, qty=1.0, trade_side="long", constraints=no_fee)
    pnl_no_fee = (exit_no_fee.price - entry_no_fee.price) * 1.0 - entry_no_fee.fee_usd - exit_no_fee.fee_usd

    entry_fee = adapter.submit_entry(symbol="XRP/USDT", raw_price=100.0, qty=1.0, trade_side="long", constraints=with_fee)
    exit_fee = adapter.submit_exit(symbol="XRP/USDT", raw_price=101.0, qty=1.0, trade_side="long", constraints=with_fee)
