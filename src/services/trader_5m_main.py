...
def main() -> None:
    ...
    emit_service_event(service="trader_5m", event="starting", runtime_timeframe=cfg.trading_5m.runtime_timeframe)

    stop_event = threading.Event()
    install_shutdown_handlers(stop_event, service="trader_5m")
    ...
    try:
        ...
        emit_service_event(service="trader_5m", event="started", venue=venue, symbols=symbols)
        ...
        while not stop_event.is_set():
            ...
            if latest_mtime_ns != config_mtime_ns:
                try:
                    ...
                    emit_service_event(service="trader_5m", event="config_reload_failed", error=str(exc))
            ...
            for symbol in symbols:
                try:
                    ...
                    session.rollback()
                    emit_service_event(service="trader_5m", event="cycle_error", symbol=symbol, error=str(exc))
        ...
        session.close()
        emit_service_event(service="trader_5m", event="stopped")
